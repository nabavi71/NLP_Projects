...
class CNN(TFModel):
    def input_layer(self):
        ...
        with tf.compat.v1.variable_scope("input_layer"):
            ...
            self.dropout = tf.compat.v1.placeholder_with_default(input=1.0, shape=(), name="dropout")
        ...
        with tf.compat.v1.variable_scope("embedding_layer"):
            if self.config.pretrained is None:
                ...
                word_embedding = tf.compat.v1.get_variable(
                    name="word_embedding",
                    dtype=tf.float32,
                    initializer=tf.compat.v1.random_uniform([self.config.n_words, self.config.dim_word],
                                                  minval=-0.25,
                                                  maxval=0.25))
            else:
                word_embedding = tf.compat.v1.get_variable(
    ...
    def CNN(self):
        with tf.name_scope("convolutional_neural_network"):
            ...
            for i, filter_size in enumerate(self.config.filter_sizes):
                with tf.compat.v1.variable_scope("ConvMax-%s" % i):
                     # Convolutional Layer
                    # Set Filter shape
                    ...
                    W = tf.compat.v1.get_variable(
                        name="ConvMax_w", initializer=tf.random.truncated_normal(self.filter_shape))
                    b = tf.compat.v1.get_variable(name="ConvMax_b",
                                        initializer=tf.constant(0.0, shape=[self.config.n_filters]))
                    # Create Convolutional model
                    self.conv = tf.compat.v1.nn.convolution(self.word_vectors, W, padding="VALID", dilation_rate=[
    ...
    def output_layer(self):
        with tf.compat.v1.variable_scope("output_layer"):
            layer = {'weights': tf.compat.v1.get_variable(name="output_w", shape=[self.total_features, 2],
